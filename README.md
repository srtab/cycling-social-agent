...
2. For each new race, generates two Portuguese drafts: one for Facebook and one for Instagram. Drafts use a per-race "feeling" note (the activity's private description in Strava) and few-shot voice examples from past posts.
...
# Reload style examples after editing the markdown file:
